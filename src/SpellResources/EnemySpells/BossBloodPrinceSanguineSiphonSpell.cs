...
public partial class BossBloodPrinceSanguineSiphonSpell : SpellResource
{
    ...
    {
        if (Boss == null || !GodotObject.IsInstanceValid(Boss) || !Boss.IsAlive)
        {
        ...
        // ── notify boss UI ────────────────────────────────────────────────────
        float healthTargetFraction = channelNode.GetHealthTargetFraction();
        (Boss as TheBloodPrince)?.OnSanguineSiphonChannelStarted(ChannelDuration, healthTargetFraction);
