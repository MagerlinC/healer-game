...
/// Layout (left → right):
///   ┌─ Shop ──────────────────────────┐  │  ┌─ Sell Items ──────────────────────┐
///   │  [icon] Stone of Rebirth   75g  │  │  │  Gold: 45g                        │
///   │         "Revive one fallen…"    │  │  │  ─────────────────────────────     │
///   │         [Buy] / SOLD OUT        │  │  │  [icon] Crystal Staff    sell: 20g │
///   └─────────────────────────────────┘  │  │  [icon] Ruler's Signet   sell: 50g │
///                                        │  └───────────────────────────────────┘
///
/// Sell: clicking a Sell button removes the item from the player's inventory
/// and adds the appropriate gold to RunState.
/// Buy: clicking Buy deducts gold and activates the Stone of Rebirth effect.
/// </summary>
public partial class MerchantPane : Control
{
    // ── colour palette (mirrors LoadoutController / EquipmentPane) ─────────
    ...
    /// Call this each time the panel is opened.
    /// </summary>
    public void Refresh()
    {
        RefreshGoldLabel();
    ...
    Control BuildShopSection()
    {
        ...
        vbox.CustomMinimumSize = new Vector2(240f, 0f);
    ...
    Control BuildStoneOfRebirthRow()
    {
        ...
        nameLabel.AddThemeColorOverride("font_color", new Color(0.80f, 0.65f, 0.95f)); // soft purple for legendary feel
    ...
    void RefreshStoneStatus()
    {
        ...
        {
            bool canAfford = RunState.Instance.Gold >= MerchantStore.StoneOfRebirthPrice;
            _stoneStatusLabel.Text = canAfford ? "" : "Not enough gold.";
            _stoneStatusLabel.AddThemeColorOverride("font_color", new Color(0.80f, 0.35f, 0.35f));
    ...
    Control BuildSellRow(EquippableItem item)
    {
        ...
        // Sell price
        int price = MerchantStore.SellPrice(item.Rarity);
        var priceLabel = new Label();
    ...
    void OnSellItem(EquippableItem item)
    {
        ...
        RunState.Instance.AddGold(price);
        Refresh();
    }
