...
public partial class MainMenuUI : Node2D
{
    ...
    // ── rebind state ──────────────────────────────────────────────────────────
    int    _rebindSlot       = -1;   // -1 = not rebinding
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        var actionName = $"spell_{_rebindSlot + 1}";
        ...
        // Update the key label in the grid
        var grid = GetTree().Root.FindChild("SettingsPanel", true, false)
            ?.FindChild("GridContainer", true, false) as GridContainer;

        if (grid != null)
        {
            // Columns: slotLabel | keyLabel | rebindBtn  → key label is at col index 1
            var rowOffset = _rebindSlot * 3 + 1;
            if (rowOffset < grid.GetChildCount() && grid.GetChild(rowOffset) is Label lbl)
                lbl.Text = GetKeybindLabel(actionName);
        }
